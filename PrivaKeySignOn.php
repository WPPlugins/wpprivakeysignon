...
/*
	...
	Description: This plugin allows users to sign into their WordPress accounts with the authentication service PrivaKey.
	Version: 1.0.7
	Author: Probaris
				...
				if ($guid == null) {
					...
					if ($binding){
						...
						if (count($rows) == 0){
							...
							error_log('bound user to guid: ' . $guid);
						...
						if (get_user_meta($currentID, 'privakey_guid', true) == $guid){
							...
							error_log('unbound user from guid: ' . $guid);
